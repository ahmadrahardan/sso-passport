                ...
                <div class="p-6 text-gray-900">
                    ...
                    @if (session('new_client_id') && session('new_client_secret'))
                        <div class="mb-6 p-4 bg-blue-100 border-l-4 border-blue-500 text-blue-700">
                            ...
                            <p class="text-sm">Berikut adalah Client ID dan Client Secret Anda. **Harap simpan Client
                                Secret ini di tempat yang aman. Anda tidak akan bisa melihatnya lagi.**</p>
                            <div class="mt-3 p-3 bg-gray-50 rounded-md">
                                <div class="mb-2">
                                    <label class="block font-medium text-sm text-gray-700">Client ID</label>
                                    <input type="text" readonly value="{{ session('new_client_id') }}"
                                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm">
                                </div>
                                <div>
                                    <label class="block font-medium text-sm text-gray-700">Client Secret</label>
                                    <input type="text" readonly value="{{ session('new_client_secret') }}"
                                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm">
                                </div>
                            </div>
                        </div>
...
<table border="1" cellpadding="10">
    ...
    @foreach ($clients as $client)
        <tr>
            ...
            <td> <a href="{{ route('clients.edit', $client->id) }}">Edit</a>
                <form action="{{ route('clients.destroy', $client->id) }}" method="POST" style="display:inline;"> @csrf
                    @method('DELETE') <button type="submit" onclick="return confirm('Hapus client?')">Delete</button>
                </form>
            </td>
        </tr>
    @endforeach
