<div class="flex flex-col h-full">
    ...
    <div class="mb-8 text-center">
        ...
        <p class="text-gray-600 text-sm px-4">
            Lupa reset password atau lupa username email tersebut, silahkan untuk digunakan bersama kemudahan akses
            untuk berkomunikasi lewat email anda sebelumnya.
        </p>
    </div>
    ...
    <form action="{{ route('password.email') }}" method="POST" id="forgotPasswordForm" class="space-y-6">
        ...
        @if (session('status'))
            <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                class="flex items-center justify-between w-full bg-emerald-500 text-white px-4 py-3 rounded-lg shadow mb-4">
        ...
        @error('email')
            <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                class="flex items-center justify-between w-full bg-red-500 text-white px-4 py-3 rounded-lg shadow mb-4">
